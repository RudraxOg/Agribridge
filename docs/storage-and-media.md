...
The listing wizard checks claimed and detected MIME types, file size and count before saving the Blob to IndexedDB. Images may be compressed in a Web Worker. Small configured uploads request a one-use signed upload token from `/api/uploads/token`; files at or above 6 MiB use TUS with resume fingerprints and retry delays. After transfer, `/api/uploads/complete` verifies that the object exists and creates pending MIME, scan and listing-derivative jobs. The authenticated cron worker atomically claims jobs, verifies the client checksum and MIME, refuses live scanning when no approved scanner is configured, and processes dependencies in order. Only a clean listing image produces a bounded, metadata-free WebP in `public-listing-media`; the private original remains separate. The local demo scanner is deterministic and clearly non-production.
