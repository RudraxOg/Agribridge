...
export function RoleSwitcher({ locale }: { locale: string }) {
  const [loading, setLoading] = useState<string>(); const hydrated=useSyncExternalStore(()=>()=>undefined,()=>true,()=>false);const router = useRouter();
  ...
  return <div data-role-switcher data-hydrated={hydrated} className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{roles.map((role) => { const Icon = role.icon; return <Card key={role.id} className="flex flex-col p-5 transition-shadow duration-200 hover:shadow-[var(--shadow-md)]"><span className="mb-5 grid size-12 place-items-center rounded-xl bg-[var(--surface-muted)] text-[var(--forest)]"><Icon aria-hidden size={24} /></span><h2 className="text-xl font-black">{role.title}</h2><p className="mt-2 mb-6 flex-1 text-sm text-[var(--text-muted)]">{role.description}</p><Button onClick={() => choose(role)} disabled={!hydrated||!!loading}>{loading === role.id && <Loader2 className="animate-spin" aria-hidden size={18} />}Continue as {role.title}</Button></Card>; })}</div>;
}
